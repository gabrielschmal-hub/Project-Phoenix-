...
Deno.serve(async (req: Request) => {
  ...
  if (rows.length) {
    ...
    // intraday trail: one point per ticker per run, pruned to 3 days.
    // This is what the tape sparklines draw — and later, the S4 composite.
    const hist = rows.map((r) => ({
      ticker: r.ticker, ts: r.updated_at, last: r.last }));
    const { error: herr } = await SB.from("prices_intraday").insert(hist);
    if (herr) console.error("intraday insert:", herr.message);
    await SB.from("prices_intraday").delete()
      .lt("ts", new Date(Date.now() - 3 * 864e5).toISOString());
  }
