...
def fetch_bulk_bars(tickers, period, interval, label=""):
    """
    Bulk OHLCV via yf.download — the SAME endpoint that already pulls 2,893
    tickers for the screener. One call per ~250 tickers instead of one per
    ticker, so Yahoo doesn't throttle us into empty responses.

    Retries the misses in smaller batches, same as the daily pull.
    Returns {ticker: [{date,o,h,l,c,v}, ...]}
    """
    import yfinance as yf
    import time
    out = {}
                ...
                try:
                    ...
                    for dt, r in sub.iterrows():
                        try:
                            ...
                            if _isnan(c):
                                continue
                            o = float(r["Open"]);  o = c if _isnan(o) else o
                            h = float(r["High"]);  h = c if _isnan(h) else h
                            lo = float(r["Low"]);  lo = c if _isnan(lo) else lo
                            v = r["Volume"]
                            rows.append({"date": dt.strftime("%Y-%m-%d"),
                                         "o": round(o, 2), "h": round(h, 2),
                                         "l": round(lo, 2), "c": round(c, 2),
                                         "v": int(float(v)) if not _isnan(v) else None})
                        except Exception:
                            continue
                    if rows:
                        out[t] = rows
    ...
    for attempt, chunk in enumerate((200, 80, 40), start=1):
        if not remaining:
            break
        if attempt > 1:
            print(f"[research]   {label} retry {attempt}: {len(remaining)} missing")
            time.sleep(2)
        got_pass = set()
        for i in range(0, len(remaining), chunk):
            got_pass |= pull(remaining[i:i + chunk])
        remaining = [t for t in remaining if t not in out]
        if not got_pass:
            break
    print(f"[research] {label} bars: {len(out)}/{len(tickers)} tickers")
    return out


def run_research(tickers=None):
    """
    Write per-ticker chart files + a fundamentals index.

    ARCHITECTURE (rebuilt — the old one was the reason "some stocks work,
    others don't"):
      charts  -> BULK yf.download for EVERY candidate, split into one small
                 file per ticker: outputs/charts/TK.json (~5KB each).
                 The browser fetches exactly one, on click. No 3MB payload,
                 no per-ticker throttling, no 120-ticker cap.
      funds   -> per-ticker calls (info/financials/ratings) are unavoidable and
                 DO throttle, so they stay bounded to the top N and are written
                 to research_data.json. A missing fundamentals block degrades the
                 tiles only — the chart still renders.
    """
    try:
        import yfinance as yf
    except Exception as e:
    ...
    if not tickers:
        print("[research] no candidates in stocks.json; run the stock engine first")
        write_json("research_data", {"asof": _now(), "tickers": {}})
        return {}

    import os, json, time
    print(f"[research] chart universe: {len(tickers)} tickers (bulk endpoint, no cap)")

    # ---------- 1. BULK CHARTS for everything ----------
    daily = fetch_bulk_bars(tickers, "1y", "1d", "daily")
    weekly = fetch_bulk_bars(tickers, "5y", "1wk", "weekly")
    monthly = fetch_bulk_bars(tickers, "10y", "1mo", "monthly")
    ...
    written, empty = 0, []
    for tk in tickers:
        d, w, m = daily.get(tk, []), weekly.get(tk, []), monthly.get(tk, [])
        if not (d or w or m):
            empty.append(tk)
            continue
        last = d[-1]["c"] if d else (w[-1]["c"] if w else None)
        prev = d[-2]["c"] if len(d) > 1 else None
        hi52 = max((b["h"] for b in d), default=None)
        lo52 = min((b["l"] for b in d), default=None)
        # Compact parallel-array encoding: ~40% smaller than dict-per-bar.
        # 551 tickers x 3 timeframes is 21MB verbose vs 13MB compact, and this
        # file is rewritten every run — verbose would bloat git history fast.
        payload = {
            "ticker": tk, "asof": _now(),
            "quote": {"last": last, "prev": prev},
            "range52": {"low": lo52, "high": hi52},
            "chart": {"daily": _compact(d), "weekly": _compact(w), "monthly": _compact(m)},
        }
        # filesystem-safe name (BRK.B -> BRK-B)
        safe = tk.replace("/", "-").replace(".", "-")
        with open(os.path.join(charts_dir, f"{safe}.json"), "w") as f:
            json.dump(payload, f, separators=(",", ":"))
        written += 1

    print(f"[research] wrote {written} chart files to outputs/charts/")
    if empty:
        print(f"[research] no bars for {len(empty)}: {empty[:12]}{' ...' if len(empty) > 12 else ''}")

    # ---------- 2. BOUNDED FUNDAMENTALS ----------
    pinned = _pinned_tickers()
    ranked = _ranked_candidates()
    fund_list = [t for t in ranked if t in daily or t in weekly][:RESEARCH_FUND_TOP_N]
    for t in sorted(pinned):
        if t not in fund_list:
            fund_list.append(t)
    print(f"[research] fundamentals for top {len(fund_list)} (per-ticker calls throttle; charts are unaffected)")

    out = {}
    fund_ok = 0
    for idx, tk in enumerate(fund_list, 1):
        ...
        try:
            ...
            # fall back to bulk bars when the per-ticker call throttles
            if last is None and daily.get(tk):
                last = daily[tk][-1]["c"]
            if prev is None and len(daily.get(tk, [])) > 1:
                prev = daily[tk][-2]["c"]
            ...
            if lo is None and daily.get(tk):
                lo = min(b["l"] for b in daily[tk])
            if hi is None and daily.get(tk):
                hi = max(b["h"] for b in daily[tk])
            entry["range52"] = {"low": lo, "high": hi}
                ...
                if qf is not None and qf.shape[1] > 0:
                    ...
                    for col in list(qf.columns)[:6]:
                        def g(key):
                            try:
                                v = qf.loc[key, col]
                                return round(float(v) / 1e9, 3) if v is not None and not _isnan(v) else None
                            except Exception:
                                return None
                        rev = g("Total Revenue")
                        ni = g("Net Income")
                        rows.append({
                            "q": col.strftime("%Y-Q%m") if hasattr(col, "strftime") else str(col)[:7],
                            "revenue_B": rev, "net_income_B": ni,
                            "net_margin": round(ni / rev * 100, 1) if (rev and ni is not None and rev != 0) else None,
                        })
                    entry["financials"] = list(reversed(rows))
            except Exception:
                pass
            ...
            out[tk] = entry
            if idx % 25 == 0:
                print(f"[research]   fundamentals {idx}/{len(fund_list)} ({fund_ok} with info)")
            time.sleep(0.6)
        except Exception as e:
            out[tk] = entry
            continue

    payload = {"asof": _now(), "tickers": out,
               "chart_index": sorted(set(daily) | set(weekly) | set(monthly)),
               "meta": {"charts_written": written,
                        "chart_universe": len(tickers),
                        "fundamentals_pulled": len(out),
                        "fundamentals_with_info": fund_ok}}
    write_json("research_data", payload)
    print(f"[research] done: {written} charts, {len(out)} fundamentals ({fund_ok} with info)")
    return payload
...
def _pull_daily_batch(batch, days, out):
    """One yf.download call. Appends to `out`. Returns the set of tickers that came back."""
    import yfinance as yf
    ok = set()
    try:
        ...
        for t in batch:
            try:
                ...
                for dt, row in sub.iterrows():
                    rows.append((dt.strftime("%Y-%m-%d"), round(float(row["Close"]), 4), float(row["Volume"])))
                if rows:
                    out[t] = rows
                    ok.add(t)
            except Exception:
...
def fetch_daily_bars_yfinance(tickers, days=2):
    """
    Pull the last few DAILY bars for the given tickers.

    PARTIAL SUCCESS IS SUCCESS. Yahoo routinely drops a slice of a large batch.
    The old code threw away the whole pull when coverage fell under 80% — which
    silently froze the screener on a stale snapshot for weeks. We now retry the
    missing names in smaller batches and keep everything we get.

    Returns (bars_dict, unusable_flag, got, failed).
      unusable_flag is True only if coverage is genuinely broken (<25%), i.e.
      Yahoo is down/blocking — not merely incomplete.
    """
    import time
    out = {}
    total = len(tickers)
    ...
    remaining = list(tickers)
    # Pass 1: big batches (fast). Pass 2/3: smaller batches for whatever missed.
    for attempt, chunk in enumerate((250, 100, 50), start=1):
        if not remaining:
            break
        if attempt > 1:
        ...
        for i in range(0, len(remaining), chunk):
            ...
            got_this_pass |= _pull_daily_batch(batch, days, out)
        remaining = [t for t in remaining if t not in out]
        # if a whole pass added nothing, further retries won't help
    ...
    # Only unusable if the pull is truly broken, not just incomplete.
    unusable = cov < 0.25
...
def run_stocks(auto_pull=True):
    ...
    if auto_pull:
        try:
            ...
            print(f"[stocks] pulling last 2 days for {len(tickers)} tickers (history stays static)...")
            daily, unusable, got, failed = fetch_daily_bars_yfinance(tickers)
            coverage = round(got / len(tickers) * 100, 1) if tickers else 0
            print(f"[stocks] daily pull: {got} ok, {failed} failed ({coverage}% coverage)")
            if unusable:
                # Genuinely broken (<25%) — Yahoo down or blocking. Fall back.
...
def run_full():
    ...
    # Stocks BEFORE research — research derives its ticker universe from stocks.json.
    try:
        run_stocks()
    except Exception as e:
        warnings.append(f"stocks failed: {e}")
        print(f"[stocks] FAILED (non-fatal): {e}")
    # Rich per-ticker data (charts/financials/ratings) for the ticker pages.
    # The browser can't fetch Yahoo directly (CORS), so this file IS the charts.
    try:
        run_research()
    except Exception as e:
