        ...
        if not force and not next_dates and os.path.exists(stamp_path):
            try:
                if open(stamp_path).read().strip() == digest:
                    n = len([f for f in os.listdir(fin_dir) if f.endswith(".json")])
                    print(f"[fin] source unchanged, no new earnings — {n} files current, skipping")
                    return 0
            except Exception:
                pass
...
def run_gex_stocks(tickers=None):
    ...
    if tickers is None:
        ...
        if not os.path.exists(p):
            print("[gexs] no gex_universe.json — Stage 0 must run first; skipping")
            return None
        try:
            ...
            return None
        tickers = [r["ticker"] for r in uni if r.get("eligible_provisional")
                   and r["ticker"] not in ("SPY", "QQQ", "IWM")]
    if not tickers:
        print("[gexs] no eligible tickers")
        return None
