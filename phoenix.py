...
def _vbars(closes, vols, n=9):
    """
    Volume sparkline for the last n weeks: [[volume, up_flag], ...] oldest->newest.
    up_flag = 1 if that week closed >= the prior week, else 0. Volume rounded to
    2 decimals (millions of shares, as stored). Matches the old dashboard's vbar().
    """
    m = min(n, len(vols), len(closes))
    if m == 0:
        return []
    v_slice = vols[-m:]
    c_slice = closes[-m:]
    # prior close for the first bar in the window (fall back to itself if unavailable)
    prev0 = closes[-m - 1] if len(closes) > m else c_slice[0]
    bars = []
    for i in range(m):
        prev = c_slice[i - 1] if i > 0 else prev0
        up = 1 if c_slice[i] >= prev else 0
        bars.append([round(v_slice[i], 2), up])
    return bars
