...
def run_congress_trades():
    ...
    def emit(tx, member, chamber, reported):
        ...
        if not tk or tk in ("--", "N/A", "NONE", "") or tk not in uni_tickers:
            return None
        ...
        for rec in raw:
            ...
            if isinstance(nested, list):
                ...
                if out:
                    by_ticker.setdefault(out[0], []).append(out[1])
                    n_trades += 1; got_here += 1
        print(f"[congress] {url.split('/')[2]}: {len(raw)} records -> {got_here} usable trades")
...
def run_institutional_13f(identity=None, quarters=5):
    ...
    for mgr, cik in SMART_MONEY["managers"].items():
        try:
            filings = list(Company(cik).get_filings(form="13F-HR"))[:quarters]
            snaps = []
            for f in filings:
                try:
                    o = f.obj()
                    d = qdate(o)
                    if d:
                        snaps.append((d, holdings_map(o)))
                except Exception:
                    continue
            if len(snaps) < 2:
                continue
            snaps.sort(key=lambda x: x[0], reverse=True)   # newest first
            mgrs_ok += 1
            ...
            for i in range(len(snaps) - 1):
                ...
                for key, p in prv.items():
                    if key in cur or not p.get("ticker") or p["ticker"] not in universe:
                        continue
...
def run_full():
    ...
    try:
        ...
        warnings.append(f"OGE failed: {e}")
        print(f"[oge] FAILED (non-fatal): {e}")
    try:
