...
def run_house_ptr(year=None, max_filings=None):
    ...
    max_filings = int(max_filings or os.environ.get("HOUSE_PTR_MAX", "220"))
    ...
    for f in filings:
        ...
        for i, ln in enumerate(lines):
            ...
            amt = AMT.search(window)
            side = None
            if re.search(r"\bP\b(?!\w)", window) and "urchase" not in window:
                side = "buy"
            if "urchase" in window:
                side = "buy"
            if "Sale" in window or "sale" in window:
                side = "sell"
            if not dts or not side:
                continue
            d = _fd(dts[0])
            if not d:
                continue
            by_ticker.setdefault(tk, []).append({
                ...
                "chamber": "House",
                "side": side,
                "amount": (amt.group(1).replace("  ", " ") if amt else ""),
    ...
    print(f"[house] merged {added} new House transactions; "
          f"file now has {len(merged)} tickers")
    return added
...
def run_oge_disclosures():
    ...
    def to_ticker(asset):
        ...
        if m and m.group(1) in universe:
            return m.group(1)
        return _cusip_ticker_from_universe(asset, by_name)
    ...
    if not added:
        print(f"[oge] parsed {parsed}/{len(files)} file(s), 0 new mappable trades "
              f"({unmapped} assets outside the universe)")
        return None
    for tk in merged:
        merged[tk].sort(key=lambda x: x.get("date", ""), reverse=True)
    total = sum(len(v) for v in merged.values())
    ...
    print(f"[oge] +{added} executive-branch trades from {parsed} filing(s) "
          f"({unmapped} assets not in universe) — congress_trades.json now {total} trades")
    return merged
...
def run_full():
    ...
    step("gex",            run_gex_from_briefing)
    step("gex_universe",   run_gex_universe)   # feeds the screener - never optional
    step("gex_stocks",     run_gex_stocks)     # feeds the screener - never optional
