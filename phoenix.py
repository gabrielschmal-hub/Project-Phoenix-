...
def run_gex_cboe(symbol="_SPX", label="SPX"):
    ...
    OSI = re.compile(r"([A-Z^_]+)(\d{6})([CP])(\d{8})$")
    today = date.today()
    lo, hi = spot * 0.85, spot * 1.15          # +/-15% is where the gamma lives
    strikes, skipped = {}, 0
    ...
    payload = {
        ...
        "by_strike": [
            {"strike": round(k, 0),
             "net_M": round(net[k] / 1e6, 2),
             "call_M": round(strikes[k]["call"] / 1e6, 2),
             "put_M": round(strikes[k]["put"] / 1e6, 2),
             "call_oi": int(strikes[k]["call_oi"]),
             "put_oi": int(strikes[k]["put_oi"])}
            for k in ks
        ],
    }
