...
# ============================================================
# How many top-ranked names get the deep research pull each run.
RESEARCH_TOP_N = 120

def _watchlist_tickers():
    """
    Tickers worth the deep per-ticker pull.

    WHY THIS CHANGED: this used to read ONLY research.json / trades.json —
    files the frontend never commits (the watchlist lives in the browser).
    So it always found nothing, wrote an empty research_data.json, and every
    ticker page said "no data yet, wait for the engine". Waiting never helped.

    The browser cannot fetch Yahoo directly (CORS blocks
    query1.finance.yahoo.com), so this file is the ONLY way a ticker page gets
    a chart. It must never be empty.

    Now: top-ranked screener names + breakouts, PLUS anything on a committed
    watchlist / trades log. Bounded by RESEARCH_TOP_N.
    """
    import json, os
    tickers, pinned = set(), set()

    # 1) anything you've explicitly committed (optional — these are pinned first)
    for name in ("research", "trades"):
        ...
        if os.path.exists(path):
            try:
                ...
                continue
    # trades_log.json uses a different shape ({trades:[{ticker,status}]})
    ...
    # 2) the screener's best names — this is what makes the ticker pages work
    ranked = []
    ...
    if os.path.exists(path):
        try:
            d = json.load(open(path))
            stocks = d.get("stocks", [])
            # breakouts first, then by rank
            breakouts = [s["ticker"] for s in stocks if s.get("breakout") and s.get("ticker")]
            byrank = [s["ticker"] for s in stocks if s.get("ticker")]
            seen = set()
            for t in breakouts + byrank:
                if t not in seen:
                    seen.add(t); ranked.append(t)
        except Exception:
            pass

    tickers |= pinned
    for t in ranked:
        if len(tickers) >= RESEARCH_TOP_N:
            break
        tickers.add(t)

    print(f"[research] universe: {len(pinned)} pinned + top-ranked -> {len(tickers)} tickers")
    return sorted(tickers)


def run_research(tickers=None):
    """
    Pull rich data for each watchlist ticker and write research_data.json:
      { asof, tickers: { TK: { profile, quote, range52, pe, chart{daily,weekly,monthly},
                               financials[], ratings, earnings } } }
    Best-effort per field. Skips cleanly if yfinance unavailable.
    """
    try:
        import yfinance as yf
    except Exception as e:
    ...
    if not tickers:
        print("[research] no watchlist tickers; writing empty research_data.json")
    ...
    for tk in tickers:
        entry = {"profile": {}, "quote": {}, "range52": {}, "pe": None,
                 "chart": {}, "financials": [], "ratings": None, "earnings": {}}
        try:
            t = yf.Ticker(tk)
            # ---- price history: daily (1y), weekly (5y), monthly (max ~10y) ----
            def bars(period, interval):
                try:
                    ...
                    for dt, row in df.iterrows():
                        try:
                            res.append({"date": dt.strftime("%Y-%m-%d"),
                                        "o": round(float(row["Open"]), 2), "h": round(float(row["High"]), 2),
                                        "l": round(float(row["Low"]), 2), "c": round(float(row["Close"]), 2),
                                        "v": int(float(row["Volume"])) if not _isnan(row["Volume"]) else None})
                        except Exception:
                            continue
                    return res
                except Exception:
                    return []
            # Keep the payload sane: 120 tickers x 3 timeframes. 1y daily is the
            # working chart; weekly/monthly give context without 10y of noise.
            entry["chart"]["daily"] = bars("1y", "1d")
            entry["chart"]["weekly"] = bars("3y", "1wk")
            entry["chart"]["monthly"] = bars("10y", "1mo")

            # ---- info: profile, quote, pe, 52w range ----
            info = {}
            try:
                info = t.info or {}
            except Exception:
            ...
            entry["profile"]["forward_pe"] = info.get("forwardPE")
            entry["profile"]["div_yield"] = info.get("dividendYield")

            # ---- quarterly financials (revenue, net income, margins) ----
            try:
                ...
                if qf is not None and qf.shape[1] > 0:
                    ...
                    for col in list(qf.columns)[:6]:
                        ...
                        rows.append({"q": col.strftime("%Y-%m") if hasattr(col, "strftime") else str(col)[:7],
                                     "revenue_B": rev, "net_income_B": ni, "net_margin": margin})
                    entry["financials"] = list(reversed(rows))  # oldest -> newest
            except Exception:
                entry["financials"] = []
            ...
            try:
                ...
                entry["ratings"] = None
            # target price from info as a fallback signal
            entry["profile"]["target_mean"] = info.get("targetMeanPrice")
            entry["profile"]["recommendation"] = info.get("recommendationKey")
            ...
            if done % 10 == 0 or done == len(tickers):
                print(f"[research] {done}/{len(tickers)} ... {tk}: "
                      f"{len(entry['chart'].get('daily',[]))}d bars, "
                      f"{len(entry['financials'])} quarters, ratings={'y' if entry['ratings'] else 'n'}")
            time.sleep(0.5)  # gentle on Yahoo — this pull is bigger now
        except Exception as e:
            print(f"[research] {tk} failed: {e}")
            out[tk] = entry  # partial/empty entry rather than dropping

    write_json("research_data", {"asof": _now(), "tickers": out})
    print(f"[research] wrote outputs/research_data.json ({len(out)} tickers)")
    return out
