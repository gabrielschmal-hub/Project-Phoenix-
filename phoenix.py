...
def write_charts(daily_ohlcv, weekly_csv=None, tickers=None):
    """
...
def run_research(tickers=None):
    """
    OPTIONAL per-ticker fundamentals: info, quarterly financials, analyst ratings.

    Charts do NOT come from here any more — write_charts() builds them from the
    pull run_stocks already does. This pass is pure enrichment for the lower
    tiles. If it throttles, is skipped, or fails outright, charts and the
    screener are completely unaffected.

    Per-ticker Yahoo endpoints rate-limit hard, so this is bounded to the best
    names and paced. Nothing here blocks the dashboard.
    """
    try:
        import yfinance as yf
    except Exception as e:
                    ...
                    for col in list(qf.columns)[:6]:
                        def g(key):
                            try:
                                ...
                                return None
                        rev, ni = g("Total Revenue"), g("Net Income")
                        rows.append({
