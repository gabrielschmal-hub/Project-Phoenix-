...
def _held_and_planned_tickers():
    ...
    import json, os
    out = []
    for p in ("outputs/trades_seed.json", "outputs/portfolio.json"):
        if not os.path.exists(p):
            continue
        try:
            d = json.load(open(p))
...
def run_gex_ticker(sym, out_dir=None):
    """
    Real per-ticker GEX from the same free CBOE endpoint. Equities use the plain
    ticker; indices need an underscore (_SPX, _VIX, _NDX).

    This replaces the yfinance chain, whose SPX open interest was ~500x too
    small with calls and puts mislabelled.
    """
    ...
    OSI = re.compile(r"([A-Z^_]+)(\d{6})([CP])(\d{8})$")
    today = date.today()
    lo, hi = spot * 0.80, spot * 1.20
    strikes = {}
    for c in chain:
        m = OSI.search((c.get("option") or c.get("symbol") or "").replace(" ", ""))
        if not m:
            continue
        ...
        try:
            ...
            continue
        if dte < 0 or dte > 90:          # per-name gamma lives in the front 90d
            continue
        try:
        ...
        if gam <= 0:
            ...
            if iv <= 0:
                continue
            try:
                gam = compute_greeks(spot, k, max(dte, 1) / 365.0, 0.04, 0.0, iv)[0]
            except Exception:
                continue
        if gam <= 0:
            continue
        dollar = gam * oi * 100.0 * spot * spot * 0.01
        rec = strikes.setdefault(k, {"call": 0.0, "put": 0.0, "coi": 0.0, "poi": 0.0})
        if m.group(3) == "C":
            rec["call"] += dollar; rec["coi"] += oi
        else:
            rec["put"] += dollar; rec["poi"] += oi

    if len(strikes) < 6:
        return None, f"thin chain ({len(strikes)} strikes)"
    ks = sorted(strikes)
    net = {k: strikes[k]["call"] - strikes[k]["put"] for k in ks}
    total = sum(net.values())
    cum, run = [], 0.0
    for k in ks:
        run += net[k]; cum.append(run)
    crosses = [ks[i] for i in range(1, len(cum))
               if (cum[i - 1] < 0 <= cum[i]) or (cum[i - 1] > 0 >= cum[i])]
    flip = (min(crosses, key=lambda k: abs(k - spot)) if crosses
            else ks[min(range(len(cum)), key=lambda i: abs(cum[i]))])
    res = {
        "ticker": sym, "spot": round(spot, 2), "asof": _now(),
        "source": "CBOE delayed chain (free) - real OI, front 90d",
        "stock_validity": "ok",
        "net_gex_M": round(total / 1e6, 2),
        "regime": "Positive Gamma" if total >= 0 else "Negative Gamma",
        "gamma_flip": round(flip, 2),
        # signed like the MME briefing: negative = flip sits BELOW spot
            "dist_to_flip_pct": round((flip / spot - 1) * 100, 2) if flip else None,
        "call_wall": round(max(ks, key=lambda k: strikes[k]["call"]), 2),
        "put_wall": round(max(ks, key=lambda k: strikes[k]["put"]), 2),
        "profile": [{"strike": round(k, 2), "net_M": round(net[k] / 1e6, 3),
                     "call_M": round(strikes[k]["call"] / 1e6, 3),
                     "put_M": round(strikes[k]["put"] / 1e6, 3),
                     "coi": int(strikes[k]["coi"]), "poi": int(strikes[k]["poi"])}
                    for k in ks],
    }
    if out_dir:
        safe = sym.replace("/", "-").replace(".", "-")
...
def run_gex_cboe(symbol="_SPX", label="SPX"):
    """
    Real SPX gamma from CBOE's free delayed option chain.

    This replaces the SPY x10 proxy, which was not a calibration problem but an
    instrument mismatch: SPY and SPX are different books with different dealers
    and different flow, so the proxy systematically underweights institutional
    put hedging and can put the flip below spot while the market is actually in
    negative gamma.

    No key, no login, no session - plain HTTPS, so it runs inside GitHub Actions.
    Endpoint: cdn.cboe.com/api/global/delayed_quotes/options/_SPX.json
    """
    ...
    try:
        r = requests.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=60)
        if r.status_code != 200:
            print(f"[gexcboe] HTTP {r.status_code} - keeping previous gex.json")
            return None
        j = r.json()
    except Exception as e:
    ...
    if not chain or not spot:
        print(f"[gexcboe] empty chain or no spot ({len(chain)} rows) - keeping "
              f"previous file")
        return None
    print(f"[gexcboe] {len(chain)} contracts, spot {spot:,.2f}")

    # OSI symbol: ROOT + YYMMDD + C/P + strike*1000 zero-padded to 8
    OSI = re.compile(r"([A-Z^_]+)(\d{6})([CP])(\d{8})$")
    today = date.today()
    # +/-10% and <=180 DTE. The first cut used +/-15% and 400 DTE, which swept in
    # LEAPS parked on round strikes (8000) - that single strike then won call
    # wall, put wall AND pin simultaneously, and inflated net gamma ~8x versus
    # the briefing. Dealer-relevant gamma lives in the front months near spot.
    lo, hi = spot * 0.90, spot * 1.10
    MAX_DTE = int(os.environ.get("GEX_MAX_DTE", "180"))
    strikes, skipped = {}, 0
    vanna_tot, charm_tot = 0.0, 0.0

    for c in chain:
        sym = c.get("option") or c.get("symbol") or ""
        m = OSI.search(sym.replace(" ", ""))
        if not m:
            skipped += 1
            continue
        yymmdd, cp, kraw = m.group(2), m.group(3), m.group(4)
        k = int(kraw) / 1000.0
        if not (lo <= k <= hi):
            continue
        try:
        ...
        if gam is None or gam <= 0:
            # CBOE usually ships greeks; if not, derive from the quoted IV
            try:
                iv = float(c.get("iv") or 0)
            except Exception:
                iv = 0.0
            ...
            T = max(dte, 1) / 365.0
            try:
                gam = compute_greeks(spot, k, T, 0.04, 0.0, iv)[0]
            except Exception:
                continue
        if not gam or gam <= 0:
            continue

        # dollar gamma per 1% move; dealers long calls / short puts by convention
        dollar = gam * oi * 100.0 * spot * spot * 0.01
        rec = strikes.setdefault(k, {"call": 0.0, "put": 0.0,
        ...
        if cp == "C":
            ...
            rec["put"] += dollar
            rec["put_oi"] += oi
        # vanna and charm from the same greeks call, so the tile is not blank
        try:
            iv2 = float(c.get("iv") or 0)
        except Exception:
            iv2 = 0.0
        if iv2 > 0:
            try:
                _g, _v, _ch = compute_greeks(spot, k, max(dte, 1) / 365.0,
                                             0.04, 0.0, iv2)[:3]
                sgn = 1.0 if cp == "C" else -1.0
                vanna_tot += sgn * _v * oi * 100.0 * spot
                charm_tot += sgn * _ch * oi * 100.0 * spot
            except Exception:
                pass

    if len(strikes) < 20:
        print(f"[gexcboe] only {len(strikes)} usable strikes ({skipped} symbols "
              f"unparsed) - keeping previous file")
        return None
    ...
    # Gamma flip = the strike where the RUNNING TOTAL of net gamma changes sign.
    # A naive scan can miss it entirely when the curve dips and recovers, so
    # build the cumulative series first and take the last sign change.
    cum, run = [], 0.0
    for k in ks:
        run += net[k]
        cum.append(run)
    # the flip that matters is the one NEAREST SPOT, so pick the crossing with
    # the smallest distance to spot rather than the first or last in the scan
    crosses = [ks[i] for i in range(1, len(cum))
               if (cum[i - 1] < 0 <= cum[i]) or (cum[i - 1] > 0 >= cum[i])]
    flip = min(crosses, key=lambda k: abs(k - spot)) if crosses else None
    if flip is None:
        # never crosses: use the strike whose cumulative total is closest to 0
        flip = ks[min(range(len(cum)), key=lambda i: abs(cum[i]))]

    call_wall = max(ks, key=lambda k: strikes[k]["call"])
    put_wall = max(ks, key=lambda k: strikes[k]["put"])
    pin = max(ks, key=lambda k: strikes[k]["call"] + strikes[k]["put"])
    up = [k for k in ks if k > spot]
    dn = [k for k in ks if k < spot]

    payload = {
        "asof": _now(),
        "source": "CBOE delayed option chain (free, no key) - real %s OI" % label,
        "symbol": label,
        "overview": {
            "spot": round(spot, 2),
            "spx_spot": round(spot, 2),      # the chart looks for spx_spot
            "net_gex_B": round(total / 1e9, 2),
            "net_vanna_B_per_volpt": round(vanna_tot / 1e9, 2),
            "net_charm_B_per_day": round(charm_tot / 1e9, 2),
            "regime": "Positive Gamma" if total >= 0 else "Negative Gamma",
            "gamma_flip": round(flip, 0),
            # signed like the MME briefing: negative = flip sits BELOW spot
            "dist_to_flip_pct": round((flip / spot - 1) * 100, 2) if flip else None,
        },
        "levels": {
            "call_wall": {"strike": round(call_wall, 0),
                          "gex_M": round(strikes[call_wall]["call"] / 1e6, 1)},
            "put_wall": {"strike": round(put_wall, 0),
                         "gex_M": round(strikes[put_wall]["put"] / 1e6, 1)},
            "pin": {"strike": round(pin, 0)},
            "deep_magnet_up": {"strike": round(max(up), 0)} if up else None,
            "deep_magnet_dn": {"strike": round(min(dn), 0)} if dn else None,
        },
        # the dashboard reads g.profile[].strike / .net_gex_B - writing
        # "by_strike"/"net_M" is why the histogram vanished
        "profile": [
            {"strike": round(k, 0),
             "net_gex_B": round(net[k] / 1e9, 4),
             "call_gex_B": round(strikes[k]["call"] / 1e9, 4),
             "put_gex_B": round(-strikes[k]["put"] / 1e9, 4),
             "coi": int(strikes[k]["call_oi"]),
             "poi": int(strikes[k]["put_oi"])}
            for k in ks
        ],
    }
    write_json("gex", payload)
    print(f"[gexcboe] net {total/1e9:+.2f}B ({payload['overview']['regime']}), "
          f"flip {flip:,.0f}, call wall {call_wall:,.0f}, put wall "
          f"{put_wall:,.0f}, {len(ks)} strikes")
    return len(ks)


def _briefing_is_readable(path):
    """
    The 'GEX_Daily_Briefing_*.pdf' files are not PDFs. They are ZIP archives of
    ~73 JPEG screenshots (header PK\\x03\\x04), so pdftotext has never been able
    to read one and the briefing branch has been failing silently since it was
    written. Say so out loud instead of quietly falling through to the proxy.
    """
    try:
    ...
    if head[:4] == b"%PDF":
        return True
    if head[:2] == b"PK":
        print(f"[gexbrief] {os.path.basename(path)} is a ZIP of images, not a "
              f"PDF - no text layer to parse. Briefing skipped; CBOE is the "
              f"live source and is fresher anyway.")
    else:
        print(f"[gexbrief] {os.path.basename(path)}: unrecognised format")
    return False
