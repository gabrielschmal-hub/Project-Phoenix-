...
def run_ratings_all(limit=None):
    ...
    order = sorted(universe.keys(),
                   key=lambda t: -(universe[t].get("market_cap") or 0))
    if limit:
        order = order[:limit]
    # load existing so we accumulate across weekly runs
    out = {}
    p = os.path.join(OUTPUTS_DIR, "ratings_all.json")
    if os.path.exists(p):
        try:
            out = json.load(open(p)).get("tickers", {})
        except Exception:
            out = {}
    got = 0
    for idx, tk in enumerate(order, 1):
        try:
            ...
            if rec is not None:
                ...
                got += 1
            if idx % 50 == 0:
                print(f"[ratall]   {idx}/{len(order)} scanned, {got} with ratings")
                # periodic checkpoint so a timeout still commits progress
                write_json("ratings_all", {"asof": _now(), "count": len(out),
                                           "tickers": out})
            time.sleep(0.5)
        except Exception:
            continue
    write_json("ratings_all", {"asof": _now(), "count": len(out), "tickers": out})
    print(f"[ratall] ratings_all.json: {len(out)} tickers with ratings")
    return out
...
def run_gex_from_briefing(path=None):
    ...
    import os, glob
    if path is None:
        cands = sorted(glob.glob("briefings/*.pdf") + glob.glob("briefings/*.PDF"),
...
def run_institutional_13f(identity="phoenix-research@example.com"):
    """
    ...
    edgartools is a free, no-API-key SEC EDGAR client. SEC requires a UA
    identity string (any contact). Non-fatal on failure.
    """
    import json, os
    try:
