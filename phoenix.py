...
def _watchlist_tickers():
    """Union of tickers in research.json and trades.json (the names worth deep data)."""
    import json, os
    tickers = set()
...
def run_research(tickers=None):
    ...
    for tk in tickers:
        ...
        try:
            ...
            entry["chart"]["weekly"] = bars("5y", "1wk")
            ...
            out[tk] = entry
            print(f"[research] {tk}: {len(entry['chart'].get('daily',[]))}d bars, "
...
def run_full():
    ...
    # Rich per-ticker research data for watchlist names (bounded, non-fatal)
    try:
        run_research()
    except Exception as e:
