...
print("== GEX wall selection (nearest, not largest) ==")
def wall_chain(spot=5000.0):
    ch=[]
    # 30 base strikes with modest OI so the degenerate guard passes
    for i in range(30):
        K = spot*0.90 + i*(spot*0.20/30)
        for kind in ("call","put"):
            ch.append({"strike":K,"T_years":0.1,"kind":kind,"open_interest":3000,"iv":0.2})
    # nearest qualifying put support 4900 (40k), deep put magnet 4600 (100k)
    ch.append({"strike":4900.0,"T_years":0.1,"kind":"put","open_interest":40000,"iv":0.2})
    ch.append({"strike":4600.0,"T_years":0.1,"kind":"put","open_interest":100000,"iv":0.2})
    # nearest qualifying call resistance 5100 (50k), deep call magnet 5400 (120k)
    ch.append({"strike":5100.0,"T_years":0.1,"kind":"call","open_interest":50000,"iv":0.2})
    ch.append({"strike":5400.0,"T_years":0.1,"kind":"call","open_interest":120000,"iv":0.2})
    return ch
g = px.gex_engine(wall_chain(), 5000.0, 1.0)
L = g["levels"]
check("tactical put wall = NEAREST qualifying below (4900, not 4600)",
      L["put_wall"]["strike"]==4900, str(L["put_wall"]))
check("tactical call wall = NEAREST qualifying above (5100, not 5400)",
      L["call_wall"]["strike"]==5100, str(L["call_wall"]))
check("deep put magnet kept separately (4600)", L["magnets"]["put"]["strike"]==4600)
check("deep call magnet kept separately (5400)", L["magnets"]["call"]["strike"]==5400)
sup=[p["strike"] for p in L["supports"]]; res=[p["strike"] for p in L["resistances"]]
check("supports ordered nearest-below -> deepest", sup==sorted(sup,reverse=True) and sup[0]==4900, str(sup))
check("resistances ordered nearest-above -> highest", res==sorted(res) and res[0]==5100, str(res))
check("small-OI strikes below threshold excluded from walls",
      all(s in (4900.0,4600.0) or False for s in []) or (4600 in sup and len(sup)==2), str(sup))
...
print("== E3b Stage 0: gex universe eval + OCC parser ==")
csv_text = "SYMBOL,CALLS,PUTS,TOTAL\nMU,400000,450000,850000\nTOTAL,400000,450000,850000\n"
check("OCC csv parser extracts volume", px._parse_occ_volume_csv(csv_text, "MU")==850000)
check("OCC parser returns None on garbage", px._parse_occ_volume_csv("<html>error</html>", "MU") is None)
cfg = dict(px.GEX_UNIVERSE); cfg["seed"]=["AAA","BBB","CCC"]
samples = {"AAA": {f"202607{d:02d}": 200000 for d in range(1,11)},   # 10 sessions
           "BBB": {"20260701": 150000}}                                # 1 session
shares = {"AAA": 5_000_000, "BBB": 5_000_000, "CCC": 5_000_000}
seed = {"BBB": 160000, "CCC": 50000}
u = px.evaluate_gex_universe(samples, shares, cfg, seed)
m = {r["ticker"]: r for r in u}
check("OCC preferred at >=8 samples", m["AAA"]["source"].startswith("OCC") and m["AAA"]["data_depth"]=="provisional")
check("IBKR seed used when OCC shallow", "IBKR" in m["BBB"]["source"] and m["BBB"]["eligible_provisional"])
check("seed below Rule 2 floor fails", not m["CCC"]["eligible_provisional"])
check("ratio math: 200k contracts vs 5M shares = 400%", m["AAA"]["opt_to_shares_ratio_pct"]==400.0)
check("ranked by ratio", [r["rank"] for r in u]==[1,2,3])

print("== earnings: never-seen tickers enter the due queue ==")
q = {"OLD": quarters()}
rows_, dates_, checked_, changed_ = [], {}, [], set()
due = px._earnings_due_queue(q)
# simulate the head-of-queue injection logic
extra = [t for t in ["NEWT","OLD"] if t not in q]
due2 = [(t,9999) for t in extra] + [d for d in due if d[0] not in set(extra)]
check("ticker with no quarterly history becomes due", ("NEWT",9999) in due2)


print("== calibration backfill: pairing + regime-side agreement ==")
src_rows = [{"date":"2026-07-03","net_gex":17.78,"vanna":51.2,"charm":18.4,"flip":7464.04,"spot":7483.25},
            {"date":"2026-07-17","net_gex":-14.11,"vanna":47.0,"charm":13.7,"flip":7495.40,"spot":7457.69},
            {"date":"2026-07-10","net_gex":5.0,"vanna":None,"charm":None,"flip":None,"spot":None}]
eng = {"2026-07-03":{"net_gex_B":2.1,"net_vanna_B":6.0,"net_charm_B":2.2,"flip":7440.0,"spot":7483.0},
       "2026-07-17":{"net_gex_B":1.4,"net_vanna_B":5.5,"net_charm_B":1.9,"flip":7450.0,"spot":7458.0}}
pairs, missing = px._pair_calib_entries(src_rows, eng)
check("pairs matched by date", len(pairs)==2 and missing==["2026-07-10"])
check("pair carries both sides", pairs[0]["engine_flip"]==7440.0 and pairs[0]["source_flip"]==7464.04)
n, agree, bad = px._flip_side_agreement(pairs)
# 07-03: source flip below spot, engine flip below spot -> agree
# 07-17: source flip ABOVE spot, engine flip BELOW spot -> DISAGREE (the real divergence)
check("side agreement computed", n==2 and agree==1 and bad==["2026-07-17"], f"{n},{agree},{bad}")
...
print("== VIX term structure validator ==")
check("vix guard passes 4 points", px.write_json_guarded.__name__=="write_json_guarded")  # presence
good = {"futures":[{"label":"9D","value":15},{"label":"30D","value":16},{"label":"3M","value":17}]}
bad  = {"futures":[{"label":"30D","value":16}]}
import tempfile as _tf, shutil as _sh, os as _os
_t=_tf.mkdtemp(); _o=px.OUTPUTS_DIR; px.OUTPUTS_DIR=_t
try:
    check("3+ points writes", px.write_json_guarded("vix_term", good, lambda pl: [] if len(pl.get("futures") or [])>=3 else ["few"]))
    check("thin curve held back", not px.write_json_guarded("vix_term", bad, lambda pl: [] if len(pl.get("futures") or [])>=3 else ["few"]))
finally:
    px.OUTPUTS_DIR=_o; _sh.rmtree(_t, ignore_errors=True)
